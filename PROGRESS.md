...
- [ ] E7 Effects/captions/audio in editor + contextual inspector + QC panel + continuity review mode + AI editing actions (b-roll/coverage proposals via Director). ✓ when: tests; export honors every inspector control.
- [ ] E8 Acceptance + verification: 26-step product test (backend TestClient journey), 19-step visual QA (Playwright), Docker verify (D50 variant), README/CLAUDE (D84+)/design mirrors, full regression. ✓ when: all suites green, pushed.

## Next up
Working the Editor phases above (E1 in progress). Prior state: all earlier phases complete — 298 backend + 27 frontend tests green, sandbox container verified. First-boot steps on the user's own network: live Civitai scrape, real X login through the connect modal, first AI generation with a real fal.ai/Replicate/WaveSpeed key (see README launch checklist).
