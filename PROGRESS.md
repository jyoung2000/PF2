...
- [x] F4 Tool layer + fallback: `forge/tools.py` typed tools over the generation queue; generation start factored into a service; opt-in one-step fallback recorded on the job. ✓: 6 tests (honest availability, typed arg validation, queue round-trip with structured job status, input mapping, structured failure without silent fallback, opt-in fallback lineage). Done: `forge/tools.py` (11 tools; extra modes work the moment a catalog offer declares them), queue `_fail` hook, `/api/forge/{tools,jobs}`.
- [x] F5 Test Lab + evaluation: prompt_experiments/prompt_variants/variant_runs tables; deterministic evaluator + optional LLM critique; refinement = new version with diff. ✓: 3 tests (full A/B loop with scores/winner/latency, fork lineage + rollback, avoid→negative refinement with word diff). Done: `forge/models.py` (7 tables incl. plans/workflows for later phases), `forge/experiments.py`, `forge/evaluate.py` (vision-level checks reported unavailable, never guessed), lab routes on `/api/forge`.
- [x] F6 Creative Plans: creative_plans/plan_assets; deterministic presets + optional LLM draft as a proposal; lock/regenerate/fork/reorder; execution through tools. ✓: 2 tests (launch-campaign preset with dependency gating, edit/lock, hero output wired into dependents, rerun-failed-only, fork with remapped links). Done: `forge/planner.py` (4 presets + validated optional LLM draft), plan routes on `/api/forge`.
- [x] F7 Workflows: serialized node graph, validation, topological executor over tools + local ops, waiting_approval pause, templates incl. shorts_pipeline. ✓: 4 tests (validation of cycles/types/edges, template end-to-end through the queue, approval pause + resume feeding the approved keyframe into video, shorts pipeline with honest transcription and real ffmpeg clips). Done: `forge/workflows.py` (11 tool nodes + 8 local nodes, flag_modified JSON persistence, scheduler tick), workflow routes on `/api/forge`.
- [x] F8 Frontend Forge section: Compose (idea → intent → ranked models with reasons → package → generate), Catalog browser with capability badges, Lab (side-by-side runs, scores, versions/diffs), Plans, Workflows editor, Usage dashboard; provider settings free/local/fallback controls. ✓: build clean, 24 vitest tests, live Playwright screenshots of every Forge page with zero page errors (`forge-*.png`). Done: `lib/forge.ts` (typed client + pure helpers), `pages/forge/{ForgePage,ComposePage,CatalogPage,LabPage,PlansPage,WorkflowsPage,UsagePage}.tsx`, nav `Forge`, `forge_prefer_free` toggle in ProvidersCard, `/api/forge/usage`. Fixed en route: 'music player' no longer routes image assets to audio (intent lexicon + plan assets force their declared kind).
- [x] F9 Docs + verification: README Forge section; CLAUDE.md D82; design preview re-captured with 6 Forge screens (30 routes); canvas gains a Forge · Compose artboard (11 artboards, republished); docker sandbox smoke — legacy DB (forge tables dropped) migrated on boot with the catalog installed, empty-dir boot creates plans/workflows, state survives `docker restart`. ✓: 266 backend + 24 frontend tests green, production build clean, pushed.

## Phase 2 — upstream audit + multimodal completion (P1–P8)
- [x] P1 Audit: all 15 upstream repos cloned and read; `docs/upstream-audit.md` (files inspected, per-feature keep/adapt/add/ignore, licences); `docs/integration-ideas.md` rewritten from real source.
- [x] P2 Real multimodal execution: `generation/muapi.py` (one adapter, six upstream repos' worth of endpoints), capability families in `pricing.json`, `forge/artifacts.py` for audio/3D/text outputs, queue branch, audio/3d kinds in the capability matrix. ✓: 8 tests.
- [x] P3 Genuine evaluation: `forge/vision.py` (vision LLM → MuAPI openrouter-vision → honest none), `complete_vision` on the LLM clients, keyframe sampling, audio transcription compare, 3D structural checks, `/api/forge/evaluators`. ✓: 7 tests.
- [x] P4 Provenance-aware model intelligence: source_urls/evidence/confidence/deprecation on every entry, 11 upstream-sourced 2026 models, router reports deprecations and catalogue-only models specifically. ✓: 4 tests.
- [x] P5 Typed workflow ports (Vibe-Workflow) + ranked highlights with overlap suppression and 9:16 reframe (shorts generator). ✓: 8 tests.
- [x] P6 UI: evaluation panel (scores, backend, evidence, honest unavailable), provenance on candidates and model cards, structured errors rendered as sentences instead of raw JSON. ✓: 27 frontend tests, live browser pass.
- [x] P7 Security + Docker: secret masking/DEFAULTS registration, artifact traversal + extension allowlist, no shell=True; image now ships the catalogue; verified on a stale-catalogue legacy volume, a clean volume and across a restart. ✓: 5 tests.
- [x] P8 Docs (README, CLAUDE D83) + final regression.

## Editor — AI Filmmaking Studio + professional video editor (spec 64 sections, phases E1–E8)

### Implementation map (spec §1.6 — keep / extend / new)
**Keep as-is (already satisfies the spec):** film data core (projects/scenes/shots/takes/assets+versions/gates/events), Director proposals (never silently applied), continuity engine, per-shot takes with compare/select, start/end-frame chaining, cost estimate + budget modes, QA (ffprobe+heuristics), footage corpus + stock adapters, subtitles/audio tracks/export (conform→concat/xfade→amix→burn-in), FakeProvider deterministic mock mode, storyboard grid/contact-sheet/inspector, presets + SVG shot-type library, camera/lighting controls, media library integration (takes are Posts).
**Extend:** `film/export.py` (a built sequence replaces the storyboard-derived plan; per-clip trim/speed/effects/fades honored), `film/qa.py` (sequence checks: empty tracks, unlinked clips, missing media, overlap), TimelinePage (becomes storyboard-timing view; editor is its own page), StoryboardPage (selection sync + [Build Timeline] + review chips), `lib/film.ts` (sequence client), takes (review state approve/reject).
**New:** additive tables `film_timeline_tracks` / `film_timeline_clips` / `film_markers` / `film_revisions`; backend `film/sequence.py` (build-from-storyboard, clip CRUD/move/trim/split/ripple, validation, compute, bounded revision snapshots + undo/redo API); editor frontend at `/film/editor` (media bin | preview | inspector over multi-track timeline; DOM-rendered clips, pointer-event drag/trim/marquee, magnetic snapping w/ toggle, zoom px-per-sec, rAF playback clock over a switching `<video>`, command-stack undo/redo, shortcut registry, autosave); generation review queue; batch generation with cost preview.
**Reference repos** (patterns only unless license-compatible; audit in `docs/editor-audit.md`): seq, OpenCut, cutaway, BlueFish, storyboard-tool, storyboard-forge, ai-video-production-editor, story2video, storyboard-gen cloned + studied; `livepeer/storyboard` unreachable (private/removed — noted).

- [x] E1 Audit + reference study: repo audit, license table, implementation map (`docs/editor-audit.md`), this checklist. ✓: doc committed; 298-test suite green; 9/10 repos studied (livepeer/storyboard unreachable), licenses recorded, patterns-only for unlicensed/copyleft repos.
- [x] E2 Sequence data model + engine: 4 additive tables, `film/sequence.py` (build/CRUD/split/ripple/markers/validate/compute/revisions), 19 API routes, D61 migration coverage. ✓: 11 tests (build layout incl. gaps + audio import, rebuild-undo, overlap/lock/speed validation, split w/ speed-aware trim, plain vs ripple delete, insert-gap, markers, batch=one undo step, undo/redo stacks, take swap + track ops, migration recreates dropped tables).
- [x] E3 Clip-aware export + QC: `sequence.flatten` (topmost-track-wins, segment slicing + merge, audio items) consumed by BOTH the preview manifest endpoint and `export.plan/run_export` (mode dispatch; per-clip trim/speed/fades/gain/mute honored; in-place held-frame dissolve keeps timing exact; `_atempo` retime for audio); `sequence.qc` wired into `qa.check_project` (storyboard media FAILs downgrade when the sequence drives export). ✓: 5 tests (plan dispatch + QA keys, trim+speed+gap export ffprobe-verified 3.5s, dissolve keeps 4.0s, audio clip gain/track-mute in mix, preview manifest w/ top-track slicing + resumed trim). Effects (opacity/scale/…) land in E7 with overlay compositing.
- [x] E4 Editor frontend core: `/film/editor` (media bin | preview | inspector over the multi-track timeline), lib/editor.ts pure math (zoom ladder/ruler ticks/frame TC/both-edge snap + break-out/collision/nearest-free-slot/client flatten/shortcut registry), Timeline (DOM clips + canvas ruler, drag/trim with one-batch-per-gesture commits, marquee, snap guides, scrub, markers, track M/S/L, culling), PreviewPlayer (derived clock, 30Hz throttle, audio beds, captions, safe areas, honest decode-error surface), Inspector, MediaBin, server undo/redo + shortcuts + take picker; every edit saved per-op ("all changes saved"). ✓: 46 frontend tests (19 helper + 6 component new), build clean, Playwright journey zero page errors (sandbox Chromium lacks H.264 decode — surfaced in-player, not an app bug).
- [x] E5 Preview player + storyboard sync: player shipped in E4 (derived clock, loop/frame-step/timecode/safe-areas — it IS the animatic); this phase added the two-way selection sync (storyboard "🎬 Editor" → clip selected + playhead moved; inspector "Open shot in storyboard" back), [Build Timeline] (E4 empty state + rebuild), and clip↔take replacement with optional storyboard update ("also make it the shot's selected take"). ✓: live Playwright probe both directions, zero page errors.
- [x] E6 Review queue + generation integration: `film_takes.review` (additive), `set_review`/`review_queue` + 2 routes, editor Review button w/ pending badge, queue modal (approve / reject+note / regenerate with budget-approval + cost / compare takes / swap into timeline / reopen). Batch generation + cost preview stay on Director (production runs + estimate, S2/S3); frame chaining stays visible on the storyboard inspector. ✓: 2 backend tests, live probe green.
- [x] E7 Effects/captions/audio in editor + contextual inspector + QC: effects clamped server-side, composited in export (canvas overlay; held last frame preserved), previewed via matching CSS; caption clips authored at the playhead, previewed as overlays, always burned via generated SRT + libass; audio clip gain/mute/fades editable in inspector (mixed at export since E3); QC modal surfaces the full report with jump-to-clip; continuity stays in the storyboard inspector + QC rows; AI shot proposals stay with the Director (do-not-over-automate). ✓: 3 backend tests, ffprobe-verified export, live probe green.
- [x] E8 Acceptance + verification: `tests/test_editor_acceptance.py` (full journey: storyboard → build → trim/retime/ripple/split/markers/caption/effects → undo/redo → review decisions → QC → export ffprobe-matched to the editor's runtime → simulated restart with sequence, markers, captions, history and reviews intact); Playwright journeys (`scratchpad/editor_journey.mjs` + sync/effects/review probes) all zero page errors; Docker (D50 sandbox variant): seeded boot (sequence + review + editor page 200), edit persists across `docker restart` incl. undo history, legacy DB with editor tables + review column dropped migrates on boot and builds/reviews fine; README Editor section + changelog, CLAUDE.md D84, design mirrors updated (preview re-captured with `film-editor` — 31 screens; canvas republished with a Film Studio · Editor artboard — 12 artboards). ✓: 319 backend + 46 frontend tests green.

## Inspiration 2.0 — universal AI social research + prompt discovery (spec 212 sections, phases I8–I16)

### Audit findings (spec §0/§155 — what already holds)
Grok is ALREADY optional in the core pipeline: discovery→dedupe→score→extract→enrich→analyze→knowledge runs on the LLM factory (`run_llm`), and every "grok" reference inside intel/monitoring is an evidence label, not a dependency. What Grok exclusively owns today is *creator discovery* (D71) — that gets a provider-neutral path. X is one adapter already; `MonitoredAccount` already carries `platform`; the envelope (observed/enrichment/analysis/assertions) is source-neutral. The real gaps: no AI-browser layer, no generic social adapter, prompt parsing trapped in `x_text.py`, no research jobs, no cross-source trends/velocity, no discovery UI.

- [x] I8 Browser Intelligence foundation: `browserintel/` package — one `BrowserIntelligence` facade over three engines (existing Playwright deterministic; Browser Use py; Stagehand via a pinned Node JSON-lines worker), determinism ladder L0–L4 with `auto` picking the cheapest, domain allowlist + read-only action policy + injection-defense preamble + log sanitizer, per-day AI/browser budgets, `browser_workflows` table with cached actions + observe→repair→new-version loop, sanitized crawl diagnostics, `/api/health` browser_runtime fields, settings. ✓: 13 tests — policy allowlist/ops/sanitizer, workflow versioning+health, budgets, truthful availability, REAL playwright replay on a fixture page (incl. an off-allowlist redirect staying fenced and an injection post arriving as inert data), full break→repair→verify→v2 loop, REAL browser-use agent run with a mock LLM (write/JS/file actions absent from its registry), stagehand LLM bridge proving provider-neutrality + secret scrubbing, engines-absent degradation. Engines pinned: browser-use 0.13.10 + stagehand 4.0.2 (both MIT). Sandbox note: this Chromium build lacks `Extensions.loadUnpacked`, so Stagehand's own injection path can't run here — the engine ships a documented `--load-extension` + computed-extension-id fallback and reports the reason truthfully rather than pretending.
- [x] I9 Generic social sources: capability vocabulary expanded; `intel/prompt_parser.py` (x_text generalized: labels/fences/quotes/loose-prompt scoring/fragments+assembly/video+image components/platform hints — X delegates to it); `SocialBrowserAdapter` shared Tier-2 base; new adapters — Reddit (public JSON, Tier 1), Bluesky (public AppView API, Tier 0), YouTube (ytInitialData + description mining, Tier 1, experimental), TikTok/Instagram/Pinterest (SocialBrowserAdapter configs, session-gated, experimental); per-source extraction policies; profile isolation kept (sessions/{platform}.json). ✓: 18 tests (labelled/fenced/quoted/loose-prose scoring, precedence ladder, metadata-wins-but-keeps-evidence, thread assembly with per-fragment provenance, "prompt in comments" pointer, component vocabulary; Reddit listing/gallery/video/comment-tree + search/author through a mock transport; Bluesky search/thread/no-credentials; YouTube search + description mining + honest degradation on shape change; registry capability audit). Registry now: 15 sources — bluesky (tier 0), civitai/lexica/reddit/youtube (tier 1), x + 4 art sites + 5 config-driven social sites (tier 2). 337 backend tests green.
- [x] I10 Grok-independent discovery + monitoring: monitored accounts source-neutral (per-platform handle normalization + capability-gated polling), provider-neutral creator discovery from source search (evidence-backed, never auto-follow), grok features clearly optional-provider-only. ✓: `intel/handles.py` (per-platform normalisation for x/reddit/bluesky/youtube/tiktok/instagram/civitai + URL detection), monitoring polls each creator through THEIR OWN adapter (capability-gated, needs_setup ≠ error, HTTP sources skip the browser lock), monitoring API takes a platform (or infers it from a pasted URL), `intel/discovery.py` ranks creators from PF2's own source search with evidence + never auto-follows, `/api/inspiration/discovery/status` states plainly that Grok is not required. ✓: 9 acceptance tests across three configurations — Grok fully off, no AI provider at all, local/mock LLM only — plus source-neutral monitoring and X-still-works; existing monitoring/X tests green.
- [ ] I11 Prompt intelligence: prompt_source ladder (embedded>explicit>structured>thread>deterministic>ai_extraction>ai_inference — never downgraded), fragment provenance (`params.prompt_fragments` + assembled prompts), thread-aware assembly ("prompt in comments" signal → enrichment inspects replies), observed/reconstructed/inferred labeling, field-level evidence preserved. ✓ when: parser + enrichment tests incl. multi-fragment assembly and no-hallucination guarantees.
- [ ] I12 Creator intelligence: cross-source creator linking (`creator_links` evidence-based, never name-only merges), creator discovery API + "find similar creators", prompt-quality/workflow-richness/cross-platform additions to creator stats. ✓ when: tests.
- [x] I13 Research jobs: `research_jobs` table + `intel/research.py` orchestrator (deterministic query interpretation → source routing by capability+yield → budgeted per-source crawl with failure isolation → ingest with research provenance → rank with query relevance), pause/resume/rerun/refresh (cursors), presets, priorities, partial results with reasons, `/api/inspiration/research*` routes, report + JSON/CSV/Markdown export, CLI. ✓: `intel/query_intent.py` (rules-only interpretation with cited evidence → concrete search terms), `intel/research.py` (affinity+yield source routing, budgeted multi-source crawl with per-source failure isolation, ingest through the existing pipeline with research provenance, query-relevance ranking with relevance/best/trending/hidden-gems modes, rerun/refresh with cursors), 7 API routes incl. JSON/CSV/Markdown export and presets. ✓: 8 tests (interpretation, routing, partial-completion with a down source, query-relevance ordering, provenance, presets/control, exports, honest no-source case). 359 backend tests green.
- [ ] I14 Cross-source trends + ranking: trend velocity (window growth), cross-platform signal clustering, prompt pattern mining (recurring phrase sets w/ platform spread), engagement-snapshot growth, discovery modes (trending/best-prompts/latest/hidden-gems/workflows) with per-result "why this" explanations, query-relevance scoring component. ✓ when: statistical tests on seeded data (no LLM required).
- [ ] I15 Inspiration UX: Research tab (ResearchBar+SourceSelector+presets+live progress+results in the masonry aesthetic), discovery mode chips, workflow health, evidence/fragments view in IntelPanel, source config/status truthful states, browser-intelligence settings card, search qualifiers (source:/prompt_source:/confidence:/research:), Studio/Film handoff intact. ✓ when: build clean, vitest, live Playwright at 1440/768/375 with zero page errors.
- [ ] I16 Hardening + verification: security audit (§200 list), prompt-injection fixture test, auth lifecycle test, no-LLM + local-LLM + grok-off acceptance, 1k-post performance test, legacy-DB migration test, Docker (node runtime + pinned worker; compose still one container), docs (README, CLAUDE iron-rule update + D85, this file), final engineering report. ✓ when: full suites green, container verified three ways.

## Next up
Working Inspiration 2.0 phases above (I8 in progress). Prior state: all earlier phases complete — 319 backend + 46 frontend tests green, sandbox container verified. First-boot steps on the user's own network: live Civitai scrape, real X login through the connect modal, first AI generation with a real fal.ai/Replicate/WaveSpeed key (see README launch checklist).
