...
- [ ] I10 Grok-independent discovery + monitoring: monitored accounts source-neutral (per-platform handle normalization + capability-gated polling), provider-neutral creator discovery from source search (evidence-backed, never auto-follow), grok features clearly optional-provider-only. ✓ when: grok-off acceptance test (no key, no web session, disabled) covers discovery→extraction→classification→knowledge→UI paths; existing X tests still green.
