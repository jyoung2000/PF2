...
- [ ] I9 Generic social sources: capability vocabulary expanded; `intel/prompt_parser.py` (x_text generalized: labels/fences/quotes/loose-prompt scoring/fragments+assembly/video+image components/platform hints — X delegates to it); `SocialBrowserAdapter` shared Tier-2 base; new adapters — Reddit (public JSON, Tier 1), Bluesky (public AppView API, Tier 0), YouTube (ytInitialData + description mining, Tier 1, experimental), TikTok/Instagram/Pinterest (SocialBrowserAdapter configs, session-gated, experimental); per-source extraction policies; profile isolation kept (sessions/{platform}.json). ✓ when: fixture-driven adapter tests (search/post/thread/prompt/no-prompt/error) + parser unit tests; suite green without browser deps.
