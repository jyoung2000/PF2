...
- [ ] E1 Audit + reference study: repo audit, license table, implementation map (`docs/editor-audit.md`), this checklist. ✓ when: doc committed, suite still green.
- [ ] E2 Sequence data model + engine: 4 additive tables, `film/sequence.py` (build/CRUD/split/ripple/markers/validate/compute/revisions), API routes, D61 migration coverage. ✓ when: backend tests for build-from-storyboard, edit ops, ripple, split, undo/redo snapshots, migration.
- [ ] E3 Clip-aware export + QC: export honors a built sequence (trim/speed/effects/fades/volume per clip, multi-track audio, caption clips), sequence QC checks, preview manifest endpoint. ✓ when: export tests render trimmed/sped/faded clips correctly (ffprobe-verified durations), QC tests.
